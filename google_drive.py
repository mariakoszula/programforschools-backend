...
class GoogleDriveCommands:
    ...
    def create_directory(parent_directory_id, directory_name):
        try:
            ...
            logger.debug(f"Directory has been created with ID: {folder.get('id')}")
        except HttpError as error:
            logger.error(f"Error during creation '{directory_name}' in '{parent_directory_id}': {error}")
            return None
    ...
    def search(parent_id=config_parser.get('DocTemplates', 'google_drive_id'),
        ...
        try:
            ...
            logger.error(f"Error during search of mime_type: {error}")
        return found
    ...
    def upload_file(parent_id='1Sc6UbsrzpAfTq2pq9Ieu7VSn1CGxmGrX', path_to_file=""):
        try:
            ...
            logger.error(f"Error during uploading file '{staticmethod}' in '{parent_id}': {error}")
